...
class SpellingErrorRepository:
    ...
    def _parse_errors_per_letter_and_length(self, error_dict: dict):
        ...
        for error in error_dict.keys():
            first_letter = error[0]
            if first_letter not in errors_per_letter.keys():
    ...
    def _read(self):
        ...
        with open(self._file_path, encoding="utf-8") as file:
            for row in file:
                ...
                suggestions_ok = all(check_allowed_chars(suggestion) for suggestion in correct_spellings)
                if check_allowed_chars(misspelling) and suggestions_ok:
                    self._error_count += 1
                    self._max_error_len = max(len(misspelling), self._max_error_len)

                    if misspelling not in error_dict.keys():
